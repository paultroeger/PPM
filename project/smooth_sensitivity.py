...
def compute_smooth_sensitivity_gnmax(beta, counts, num_teachers, sigma, order):
  ...
  # The original had these arguments swapped.
  ls = compute_local_sensitivity_bounds_gnmax(counts, num_teachers, sigma,
                                              order)
...
def check_conditions(sigma, m, order):
  ...
  if sp is None:
    raise ImportError("sympy is required for symbolic condition checks.")
